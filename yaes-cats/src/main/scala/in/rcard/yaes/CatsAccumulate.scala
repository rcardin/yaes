...
/** Error accumulation utilities using Cats Semigroup typeclass and NonEmptyList.
  ...
  *
  * For extension methods like `combineErrors` and `combineErrorsS`, prefer importing from
  * the syntax package:
  * {{{
  ...
  * }}}
  */
object CatsAccumulate extends AccumulateSyntax {
  ...
  inline def mapAccumulating[E, A, B](nonEmptyList: NonEmptyList[A])(
    ...
    val resultAsList = CatsAccumulate.mapAccumulating(nonEmptyList.toList)(transform)
