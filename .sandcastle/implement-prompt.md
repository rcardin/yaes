...
4. **Verify** — run `sbt -batch --no-colors test` before committing (this compiles first, so it covers type checking). Prefer a single module or class for faster feedback, e.g. `sbt -batch --no-colors "yaes-core/testOnly *AsyncSpec"`. Fix any failures before proceeding.
