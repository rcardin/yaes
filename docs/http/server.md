...
**Requirements:**
- Java 24+ (for Virtual Threads and Structured Concurrency)
- Scala 3.7.4+
...
```scala
libraryDependencies += "in.rcard.yaes" %% "yaes-http-server" % "0.12.1"
```
...
// Using withHeader method
GET(p"/download") { req =>
  Response.ok("file content")
    .withHeader("Content-Type" -> "application/octet-stream")
    .withHeader("Content-Disposition" -> "attachment; filename=data.txt")
}
```
...
> **Note:** JSON codec libraries (circe, upickle, zio-json, etc.) are not included. Choose your preferred library and implement the `BodyCodec` trait.
...
> See [Shutdown Effect documentation](effects/shutdown.md) for more details on shutdown coordination.
...
> See [Log Effect documentation](effects/log.md) for details on log levels, formatting, and custom loggers.
...
object MyApiServer {
  ...
  val routes = Routes(
    ...
    POST(p"/users") { req =>
      ...
      val newUserId = 123
      Response.created(s"""{"id": $newUserId, "name": "New User"}""")
        .withHeader("Content-Type" -> "application/json")
        .withHeader("Location" -> s"/users/$newUserId")
    },
